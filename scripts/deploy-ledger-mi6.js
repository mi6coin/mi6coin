// Deploy MI6Coin using a Ledger device, signing exactly like depArb.js:
...
//   MI6_NAME, MI6_SYMBOL, MI6_TREASURY, MI6_INITIAL_SUPPLY (wei), MI6_MINT_INTERVAL (seconds)
//
// Run examples:
//   npx hardhat --config hardhat.config.js run scripts/deploy-ledger-mi6.js --network sepolia
//   USE_LEDGER=0 DEPLOYER_PK=0x... npx hardhat --config hardhat.config.js run scripts/deploy-ledger-mi6.js --network bsc
...
async function main() {
  ...
  const NAME = env("MI6_NAME", "MI6Coin");
  const SYMBOL = env("MI6_SYMBOL", "MI6");
  const TREASURY = env("MI6_TREASURY", fromAddress);
  const INITIAL_SUPPLY = env("MI6_INITIAL_SUPPLY", ethers.utils.parseUnits("1000000", 18).toString()); // 1,000,000 MI6
  const MINT_INTERVAL = env("MI6_MINT_INTERVAL", "0"); // keep default 0 unless specified
