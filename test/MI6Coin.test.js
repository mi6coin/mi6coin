...
describe("MI6Coin (ethers v5 tests)", function () {
  ...
  async function deploy(initialSupply = E(1_000_000), mintInterval = 0) {
    ...
    const MI6 = await ethers.getContractFactory("contracts/MI6Coin.sol:MI6Coin", owner);
    coin = await MI6.deploy(
      NAME,
      SYMBOL,
  ...
  describe("constructor", () => {
    ...
    it("reverts on zero treasury", async () => {
      ...
      const MI6 = await ethers.getContractFactory("contracts/MI6Coin.sol:MI6Coin", deployer);
      await expect(
        MI6.deploy(NAME, SYMBOL, ethers.constants.AddressZero, E(1_000_000), 0)
